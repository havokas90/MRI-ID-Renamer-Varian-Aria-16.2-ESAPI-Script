...
[assembly: AssemblyTitle("ImageIdRenamer")]
[assembly: AssemblyDescription("ARIA-hosted MR/CT image ID renaming script")]
[assembly: AssemblyProduct("ImageIdRenamer")]
...
namespace VMS.TPS
{
    ...
    public class Script
    {
        ...
        public void Execute(ScriptContext context, Window window)
        {
            if (context == null)
            {
                MessageBox.Show("Script context was not provided.", "ImageIdRenamer", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            if (patient == null)
            {
                MessageBox.Show("Open a patient in Eclipse before running ImageIdRenamer.", "ImageIdRenamer", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
            ...
            ImageIdRenamerWindow dialog = new ImageIdRenamerWindow(patient, candidates);
            if (window != null)
            {
                dialog.Owner = window;
    ...
    /// </summary>
    public class ImageIdRenamerWindow : Window
    {
        ...
        public ImageIdRenamerWindow(Patient patient, List<RenameCandidate> candidates)
            : this(patient, candidates, false)
        {
        }

        public ImageIdRenamerWindow(List<RenameCandidate> candidates)
            : this(null, candidates, true)
        {
        }

        private ImageIdRenamerWindow(Patient patient, List<RenameCandidate> candidates, bool previewOnly)
        {
            _patient = patient;
            _candidates = new ObservableCollection<RenameCandidate>(candidates);
            _previewOnly = previewOnly;

            Title = previewOnly ? "ImageIdRenamer Local Preview" : "ImageIdRenamer";
            Width = 1380;
            Height = 760;
            MinWidth = 1100;
            MinHeight = 600;
